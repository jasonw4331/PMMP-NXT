...
const IdentifyRepoHost = async (url, auth = null) => {
  if (await testGithub(url, auth)) {
    return getGithubData(url)
  } else if (await testGitlab(url, auth)) {
    return getGitlabData(url)
  } else if (await testBitbucket(url, auth)) {
  ...
  try {
    ...
    try {
      const octokit = new Octokit({
        auth,
        userAgent: 'PMMP-NXT v1.0',
        baseUrl: results.length > 2 ? results[1] : 'gitlab.com',
      })
      await octokit.rest.repos.getContent({
        owner: results.length > 2 ? results[2] : results[1],
        repo: results.length > 2 ? results[3] : results[2],
