...
(
    ...
    constraint check_recovery_email_format check (recovery_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'), -- Email format check
    constraint check_recovery_email check (recovery_email is distinct from email)
);
alter table public.users
    enable row level security;
create policy "Can view own user data." on public.users for select using ((select next_auth.uid()) = id);
create policy "Can create new user data." on public.users for insert with check ((select next_auth.uid()) = id);
create policy "Can update own user data." on public.users for update using ((select next_auth.uid()) = id);
create policy "Can delete own user data." on public.users for delete using ((select next_auth.uid()) = id);
...
-- This trigger syncs user data between NextAuth and our public.users table.
...
alter table public.software
    enable row level security;
create policy "Can view own software records." on public.software for select using ((select next_auth.uid()) = developer_id);
create policy "Can create new software records." on public.software for insert with check ((select next_auth.uid()) = developer_id);
create policy "Can update own existing software records." on public.software for update using ((select next_auth.uid()) = developer_id);
create policy "Can delete own software records." on public.software for delete using ((select next_auth.uid()) = developer_id);
...
create policy "Can create new software categories." on public.software_categories for insert with check ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "None can update software categories." on public.software_categories for update using (false);
create policy "Can delete own software categories." on public.software_categories for delete using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
...
create policy "Can create new releases if a developer role." on public.releases for insert with check ((select next_auth.uid()) = (select developer_id from software where id = software_id) and (select role from public.users where id = (select next_auth.uid())) = 'developer');
create policy "Can update own releases." on public.releases for update using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can delete own releases." on public.releases for delete using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
...
alter table public.user_followers
    enable row level security;
create policy "Can view followed users." on public.user_followers for select using ((select next_auth.uid()) = follower_id);
create policy "Can follow other users." on public.user_followers for insert with check ((select next_auth.uid()) = follower_id);
create policy "Can unfollow other users." on public.user_followers for delete using ((select next_auth.uid()) = follower_id);
...
alter table public.software_followers
    enable row level security;
create policy "Can view followed softwares." on public.software_followers for select using ((select next_auth.uid()) = follower_id);
create policy "Can follow software releases." on public.software_followers for insert with check ((select next_auth.uid()) = follower_id);
create policy "Can unfollow software releases." on public.software_followers for delete using ((select next_auth.uid()) = follower_id);
...
alter table public.notifications
    enable row level security;
create policy "Can view own notifications." on public.notifications for select using ((select next_auth.uid()) = user_id);
create policy "Can update own notifications." on public.notifications for update using ((select next_auth.uid()) = user_id);
...
(
    ...
    constraint ratings_check_user_id check (next_auth.uid() != user_id)
);

alter table public.ratings
    enable row level security;
create policy "Can view own ratings." on public.ratings for select using ((select next_auth.uid()) = user_id);
create policy "Can create new ratings." on public.ratings for insert with check ((select next_auth.uid()) = user_id);
create policy "Can update own ratings." on public.ratings for update using ((select next_auth.uid()) = user_id);
create policy "Can delete own ratings." on public.ratings for delete using ((select next_auth.uid()) = user_id);
...
alter table public.comments
    enable row level security;
create policy "Can view own comments." on public.comments for select using ((select next_auth.uid()) = user_id);
create policy "Can create new comments." on public.comments for insert with check ((select next_auth.uid()) = user_id);
create policy "Can update own comments." on public.comments for update using ((select next_auth.uid()) = user_id);
create policy "Can delete own comments." on public.comments for delete using ((select next_auth.uid()) = user_id);
...
alter table public.namespaces
    enable row level security;
create policy "Can view own namespaces." on public.namespaces for select using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can create new namespaces." on public.namespaces for insert with check ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can update own namespaces." on public.namespaces for update using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can delete own namespaces." on public.namespaces for delete using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
...
alter table public.commands
    enable row level security;
create policy "Can view own commands." on public.commands for select using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can create new commands." on public.commands for insert with check ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can update own commands." on public.commands for update using ((select next_auth.uid()) = (select developer_id from software where id = software_id));
create policy "Can delete own commands." on public.commands for delete using ((select next_auth.uid()) = (select developer_id from software where id = software_id));